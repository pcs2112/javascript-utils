...
var untreeify = function untreeify(nodes) {
  var parents = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
  return nodes.reduce(function (flattenedTree, node) {
    var deepness = parents.length;

    var nodeWithHelpers = _objectSpread({}, node, {
      deepness: deepness,
      parents: parents
    });

    if (!nodeHasChildren(node) || !isNodeExpanded(node)) {
      return [].concat(_toConsumableArray(flattenedTree), [nodeWithHelpers]);
    }

    return [].concat(_toConsumableArray(flattenedTree), [nodeWithHelpers], _toConsumableArray(untreeify(node.children, [].concat(_toConsumableArray(parents), [node.id]))));
  }, []);
};
/**
